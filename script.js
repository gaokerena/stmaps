...
map.on(L.Draw.Event.CREATED, function(e) {
  ...
  if(layer instanceof L.Marker) {
    ...
    const radiusNM = layer.getRadius()/1852; // meters -> nautical miles
...
fetch(scriptURL)
  ...
  .then(data=>{
    ...
    const pinnedLayers = new Set(); // keep track of pinned tooltips
    ...
    data.forEach(item=>{
      ...
      quads.forEach(([geom,intGeom,expGeom])=>{
        ...
        if(intGeom){
          ...
          if(intFeature){
            try{const intersection=turf.intersect(feature,intFeature);if(intersection) feature=intersection;}catch(e){console.warn("Intersection failed",e);}
          }
        }
        if(expGeom){
          const expFeature=parseGeometry(expGeom);
          if(expFeature){
            try{const diff=turf.difference(feature,expFeature);if(diff) feature=diff;else return;}catch(e){console.warn("Difference failed",e);}
          }
        }
      ...
      const layer = L.geoJSON(combined,{ color, fillColor:color, weight:2, fillOpacity:0.3 }).addTo(map);

      // Bind tooltip
      layer.bindTooltip(`<strong>${nom}</strong><br>Plafond: ${item.plafond}<br>Plancher: ${item.plancher}`, { sticky:false });

      // Hover tooltip
      layer.on('mouseover', e=>{
        const l=e.target;
        if(!pinnedLayers.has(l)) l.openTooltip(e.latlng);
      });
      layer.on('mouseout', e=>{
        const l=e.target;
        if(!pinnedLayers.has(l)) l.closeTooltip();
      });

      // Click to pin/unpin tooltip at click
      layer.on('click', e=>{
        const l=e.target;
        if(pinnedLayers.has(l)){
          pinnedLayers.delete(l);
          l.closeTooltip();
        } else {
          pinnedLayers.add(l);
